...
bool http_conn::write(){
    ...
    while(1){
        ...
        if((size_t)bytes_have_send >= m_iv[0].iov_len){     //第一个iovec头部信息的数据已发送完，发送第二个iovec数据
...
HTTP_CODE http_conn::parse_headers(char* text){                    //主状态机解析报文中的 请求头 数据
    ...
    //解析请求头部内容长度字段 Content-length:
        ...
        std::cout << "parsing head content_length is: " << m_content_length << std::endl;
    }
    //解析请求头部HOST字段 Host:
...
HTTP_CODE http_conn::parse_content(char* text){                    //主状态机解析报文中的 请求体 数据
    std::cout << "start parse_content" << std::endl;
    //判断buffer中是否读取了消息体
        ...
        if(*(p+1) == '3'){
            ...
            if (users.find(name) == users.end())  //没有重名
            {
                ...
                if (!res){
                    ...
                    LOG_ERROR("SQL 执行失败（数据库插入失败、主键冲突等）");
